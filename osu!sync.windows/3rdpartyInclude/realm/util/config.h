// Version information
#define REALM_VERSION ""

// Feature detection
#define REALM_HAVE_READDIR64 0
#define REALM_HAVE_POSIX_FALLOCATE 0
#define REALM_USE_SYSTEM_OPENSSL_PATHS 0
#define REALM_HAVE_OPENSSL 0
#define REALM_HAVE_SECURE_TRANSPORT 0
#define REALM_HAVE_PTHREAD_GETNAME 0
#define REALM_HAVE_PTHREAD_SETNAME 0
#define REALM_HAVE_BACKTRACE 0
#define REALM_MAX_BPNODE_SIZE 1000
#define REALM_BACKTRACE_HEADER <backtrace.h>
#define REALM_ENABLE_ASSERTIONS 0
#define REALM_ENABLE_ALLOC_SET_ZERO 0
#define REALM_ENABLE_ENCRYPTION 1
#define REALM_ENABLE_MEMDEBUG 0
#define REALM_ENABLE_GEOSPATIAL 1
#define REALM_VALGRIND 0
#define REALM_ASAN 0
#define REALM_TSAN 0
